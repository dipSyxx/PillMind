      ...
      <Container>
        <div className="flex h-14 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm text-[#334155]">
            ...
            <a className="hover:text-[#0EA8BC]" href="#colors">Colors</a>
            <a className="hover:text-[#0EA8BC]" href="#typography">Typography</a>
            <a className="hover:text-[#0EA8BC]" href="#components">Components</a>
            <a className="hover:text-[#0EA8BC]" href="#motion">Motion</a>
            <a className="hover:text-[#0EA8BC]" href="#a11y">A11y</a>
            <a className="hover:text-[#0EA8BC]" href="#voice">Voice</a>
            <a className="hover:text-[#0EA8BC]" href="#assets">Assets</a>
          </nav>
        </div>
      </Container>
    </header>
...
function Hero() {
  return (
    <section id="top" className="relative overflow-hidden bg-gradient-to-br from-[#12B5C9] via-[#2ED3B7] to-[#3EC7E6] py-14 text-white">
      <Container>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 md:items-center">
          <div>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-3xl md:text-4xl font-bold leading-tight text-balance">PillMind Brandbook</motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.6 }} className="mt-3 max-w-xl text-white/90">
              Visual and verbal rules so PillMind looks and sounds consistent across product, web and communications.
            </motion.p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#assets" className="rounded-[12px] bg-white px-5 py-3 text-sm font-semibold text-[#0F172A] hover:bg-white/90">Download assets</a>
              <a href="#logo" className="rounded-[12px] border border-white/70 px-5 py-3 text-sm font-semibold text-white hover:bg-white/10">Start with logo</a>
            </div>
          </div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="justify-self-center">
            <BrandCardPreview />
          </motion.div>
        </div>
      </Container>
    ...
    <div className="border-b border-[#E2E8F0] bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50">
      <Container>
        <div className="flex flex-wrap items-center gap-3 py-3 text-sm">
          {items.map((i) => (
            <a key={i.href} className="rounded-full border border-[#CBD5E1] px-3 py-1 text-[#334155] hover:border-[#0EA8BC] hover:text-[#0EA8BC]" href={i.href}>{i.label}</a>
          ))}
        </div>
      </Container>
    </div>
    ...
    <footer className="border-t border-[#E2E8F0] bg-white py-8 text-sm text-[#334155]">
      <Container>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span>Contact: <a className="text-[#0EA8BC] hover:underline" href="mailto:design@pillmind.app">design@pillmind.app</a></span>
        </div>
      </Container>
    </footer>
  );
...
function LogoBlock() {
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      {/* Clearspace */}
      <Card>
        <CardHeader title="Clearspace & Min size" subtitle="Safe area = height of letter ‘P’; min 120px logo, 24px icon" />
        <CardBody>
          <ClearspaceDemo />
        </CardBody>
      </Card>
...
function BadExample({ label, type }: { label: string; type: "stretch" | "low-contrast" | "recolor" }) {
  return (
    <div className="rounded-lg border border-[#E2E8F0] bg-white p-4 shadow-card">
      ...
      <div className="mt-3 flex items-center gap-2 text-sm text-[#DC2626]"><XCircle className="h-4 w-4"/> {label}</div>
    </div>
  );
}
...
function ColorSwatch({ swatch }: { swatch: Swatch }) {
  ...
  const handleCopy = async () => {
    try { await navigator.clipboard.writeText(swatch.hex); setCopied(true); setTimeout(() => setCopied(false), 1200); } catch {}
  };
  ...
  return (
    <button onClick={handleCopy} className="group relative rounded-lg border border-[#E2E8F0] bg-white text-left shadow-card transition hover:-translate-y-0.5">
      ...
      <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: copied ? 1 : 0, y: copied ? 0 : 4 }} className={`absolute right-2 top-2 inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] ${isLight ? "bg-[#0F172A] text-white" : "bg-white/90 text-[#0F172A]"}`}>
        <Check className="h-3.5 w-3.5" /> Copied
      </motion.div>
      <div className="absolute right-2 bottom-2 opacity-60 transition group-hover:opacity-100"><Copy className="h-4 w-4"/></div>
    </button>
  );
}
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card>
        ...
        <CardBody>
          <p className="text-sm text-[#64748B]">Load via <code>next/font</code> or Google Fonts. Use 600–700 for headings, 400–600 for body.</p>
          <div className="mt-4 space-y-4">
            ...
            <div>
              ...
              <p className="text-[#334155]">PillMind reminds you to take meds, analyzes your data, and suggests safe combinations. With your consent — and for you only.</p>
            </div>
      ...
      <Card>
        ...
        <CardBody>
          <p className="text-[#334155]">Use concise, plain language. Avoid medical jargon. Example link: <a className="text-[#0EA8BC] underline decoration-2 underline-offset-2 hover:text-[#0B95A8]" href="#">privacy policy</a>.</p>
          <p className="mt-3 text-sm text-[#64748B]">Always include the disclaimer where advice appears: <em>“PillMind does not provide medical diagnoses and does not replace a doctor’s consultation.”</em></p>
        </CardBody>
      </Card>
    </div>
  );
...
function SpecRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between rounded-md border border-[#E2E8F0] bg-white px-3 py-2"><span className="font-medium">{label}</span><span className="text-[#64748B]">{value}</span></div>
  );
}
      ...
      <Card>
        ...
        <CardBody>
          <div className="flex flex-wrap gap-3">
            <button className="h-11 rounded-[12px] bg-[#0EA8BC] px-5 font-semibold text-white transition hover:bg-[#0B95A8]">Get started</button>
            <button className="h-11 rounded-[12px] border border-[#0EA8BC] px-5 font-semibold text-[#0EA8BC] transition hover:bg-[#E6F7FA]">Learn more</button>
            <button className="h-11 rounded-[12px] px-5 font-semibold text-[#0EA8BC] transition hover:bg-[#12B5C9]/10">Ghost</button>
          </div>
        </CardBody>
      </Card>
      <Card>
        ...
        <CardBody>
          ...
          <input placeholder="you@pillmind.app" className="h-11 w-full rounded-[12px] border border-[#CBD5E1] bg-white px-4 text-[#0F172A] outline-none transition focus:border-[#0EA8BC] focus:ring-4 focus:ring-[#12B5C9]/20" />
      ...
      <Card>
        ...
        <CardBody>
          <div className="rounded-[16px] border border-[#E2E8F0] bg-white p-4 shadow-card">
            ...
            <div className="mt-2 text-xs text-[#64748B]">7‑day avg: <strong className="text-[#0EA8BC]">84%</strong></div>
          </div>
        </CardBody>
      </Card>
    </div>
      ...
      <Card>
        ...
        <CardBody>
          <div className="grid grid-cols-2 gap-4">
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="rounded-[12px] border border-[#E2E8F0] bg-white p-4 shadow-card">Fade & slide in</motion.div>
            <motion.button whileHover={{ scale: 1.03 }} className="rounded-[12px] border border-[#0EA8BC] bg-white p-4 font-semibold text-[#0EA8BC] shadow-card">Hover grow</motion.button>
          </div>
        </CardBody>
      </Card>
    </div>
      ...
      <Card>
        ...
        <CardBody>
          <ul className="list-disc space-y-2 pl-5 text-sm text-[#334155]">
            ...
            <li>All images and icons require descriptive <code>alt</code> / <code>aria-label</code>.</li>
          </ul>
        </CardBody>
      </Card>
      <Card>
        ...
        <CardBody>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div className="rounded-md border border-[#E2E8F0] bg-white p-3 text-sm"><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#DCFCE7]"><Check className="h-3 w-3 text-[#16A34A]"/></span> Use clear labels and sufficient spacing</div>
            <div className="rounded-md border border-[#E2E8F0] bg-white p-3 text-sm"><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#FEE2E2]"><XCircle className="h-3 w-3 text-[#DC2626]"/></span> Don’t rely on color alone to convey meaning</div>
          </div>
        </CardBody>
      </Card>
    </div>
...
function ExamplePair({ good, better }: { good: string; better: string }) {
  return (
    <div className="mt-3 grid grid-cols-1 gap-2">
      <div className="rounded-md border border-[#E2E8F0] bg-white p-3 text-sm"><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#FEF9C3]"><span className="text-[#A16207] font-semibold">G</span></span>{good}</div>
      <div className="rounded-md border border-[#0EA8BC] bg-[#E6F7FA] p-3 text-sm"><span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-white text-[#0EA8BC] font-semibold">B</span>{better}</div>
    </div>
  );
}
...
function AssetsBlock() {
  const assets = [
    ...
    { name: "Logo — monochrome (SVG)", href: "/brand/pillmind-logo-mono.svg" },
    ...
    { name: "One‑Page Design Profile (PDF)", href: "/brand/PillMind-Design-Profile.pdf" },
  ];
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card>
        ...
        <CardBody>
          <ul className="space-y-2 text-sm">
            {assets.map((a) => (
              <li key={a.name} className="flex items-center justify-between rounded-md border border-[#E2E8F0] bg-white px-3 py-2">
                <span className="text-[#334155]">{a.name}</span>
                <a className="inline-flex items-center gap-1 rounded-md border border-[#0EA8BC] px-2 py-1 text-[#0EA8BC] hover:bg-[#E6F7FA]" href={a.href} target="_blank" rel="noreferrer">
      ...
      <Card>
        ...
        <CardBody>
          <p className="text-sm text-[#334155]">For questions, variations, or large‑format artwork, email <a className="text-[#0EA8BC] hover:underline" href="mailto:design@pillmind.app">design@pillmind.app</a>. Do not recolor or alter the marks without written approval.</p>
          <a className="mt-3 inline-flex items-center gap-1 text-sm text-[#0EA8BC] hover:underline" href="#logo"><ExternalLink className="h-4 w-4"/> See logo rules</a>
        </CardBody>
      </Card>
    </div>
  );
...
function Code({ children }: { children: React.ReactNode }) {
  return (
    <pre className="whitespace-pre-wrap rounded-[12px] border border-[#E2E8F0] bg-[#0F172A] p-3 text-xs text-[#E2E8F0]"><code>{children}</code></pre>
  );
}
    ...
    <div className="w-full max-w-[420px] rounded-[20px] border border-white/40 bg-white/10 p-4 text-white shadow-[0_20px_60px_rgba(0,0,0,0.15)]">
      <div className="rounded-[16px] border border-white/40 bg-white/10 p-4">
        ...
        <div className="mt-4 flex gap-3">
          <button className="rounded-[12px] bg-white px-4 py-2 text-sm font-semibold text-[#0F172A] hover:bg-white/90">Get started</button>
          <button className="rounded-[12px] border border-white/70 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10">Learn more</button>
        </div>
