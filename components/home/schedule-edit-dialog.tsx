...
import { Schedule, TimeFormat, Unit, Weekday } from '@/types/medication'
...
export function ScheduleEditDialog({
  ...
  const [formData, setFormData] = useState({
    ...
    doseQuantity: schedule.doseQuantity ? (typeof schedule.doseQuantity === 'number' ? schedule.doseQuantity : parseFloat(String(schedule.doseQuantity))) : 1,
    doseUnit: schedule.doseUnit ?? 'TAB',
    startDate: schedule.startDate ? format(new Date(schedule.startDate), 'yyyy-MM-dd') : '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const timezoneChanged = timezone !== schedule.timezone
      const daysOfWeekChanged =
        JSON.stringify(formData.daysOfWeek.sort()) !== JSON.stringify(schedule.daysOfWeek.sort())
      const timesChanged = JSON.stringify(formData.times.sort()) !== JSON.stringify(schedule.times.sort())
      const criticalFieldsChanged = timezoneChanged || daysOfWeekChanged || timesChanged
      ...
      if (res.ok) {
        ...
        if (criticalFieldsChanged) {
          if (doseGen && doseGen.generated > 0) {
            toast.success(
              `Schedule updated. Regenerated ${doseGen.generated} doses${doseGen.skipped > 0 ? `, skipped ${doseGen.skipped} duplicates` : ''}.`
            )
          } else if (doseGen && doseGen.errors && doseGen.errors.length > 0) {
            toast.warning(
              `Schedule updated, but dose regeneration had issues: ${doseGen.errors.join(', ')}`
            )
          } else {
            toast.success('Schedule updated. Future doses will be regenerated automatically.')
          }
        } else {
  ...
  const handleRegenerateDoses = async () => {
    ...
    try {
      // Get prescription ID from schedule
      const scheduleRes = await fetch(`/api/schedules/${schedule.id}`)
      if (!scheduleRes.ok) {
        toast.error('Failed to fetch schedule information')
        return
      }

      const scheduleData = await scheduleRes.json()
      const prescriptionId = scheduleData.prescriptionId
      ...
      if (res.ok) {
        ...
        const totalGenerated = result.totalGenerated || 0
        const totalSkipped = result.totalSkipped || 0
        if (totalGenerated > 0) {
          toast.success(
            `Regenerated ${totalGenerated} doses${totalSkipped > 0 ? `, skipped ${totalSkipped} duplicates` : ''}`
          )
        } else {
          toast.info('No new doses generated (all doses already exist)')
        }
        onSaved()
      } else {
        const error = await res.json()
    ...
    <Drawer open={open} onOpenChange={onClose} direction="bottom">
      <DrawerContent className="max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {/* Info about dose regeneration */}
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
            <p className="font-medium mb-1">Note:</p>
            <p>
              Changing days, times, or timezone will automatically delete future scheduled doses and regenerate them.
              You can also manually regenerate doses using the button below.
            </p>
          </div>
        </div>
